...
export default function PRForm({ onSubmit, loading }) {
  ...
  const [prUrl2, setPrUrl2]       = useState("");
  ...
  function handleSubmit(e) {
    e.preventDefault();
    onSubmit({ prUrl, prUrl2, ghToken, framework, scope, coverage });
  }
    ...
    <form onSubmit={handleSubmit} className="card p-5 space-y-5">
      <div>
        ...
        <div className="space-y-1">
          <label className="label">PR #1 URL *</label>
          <input
            type="url"
        ...
        <div className="space-y-1 mt-4">
          ...
          <input
            type="url"
            className="input-field font-mono text-xs"
            placeholder="https://github.com/owner/repo/pull/43"
            value={prUrl2}
            onChange={(e) => setPrUrl2(e.target.value)}
          />
        </div>
